        ...
        if (index !== 0) {
            ...
            if (svgElement) {
                ...
                const point = `<svg id="Calque_1" data-name="Calque 1" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 50 50">
                    ...
                    <path class="cls-4" d="M36.6,8.6c-3.1-3.1-7.1-4.8-11.7-4.8s-8.6,1.7-11.7,4.8c-3.3,3.3-4.8,7.3-4.8,12.4s1.3,7.1,4.2,11.3c2.7,4.2,6.9,9,12.6,14c5.7-5.2,9.8-9.8,12.6-14,2.7-4.2,4-8,4-11.3-.2-5-1.9-9-5.2-12.4ZM33.7,20.9c0,.6,0,1.1-.2,1.6,0,.2,0,.4,0,.5,0,.4-.3.8-.5,1.2-1.4,2.9-4.5,4.8-7.9,4.8s-8.6-3.8-8.8-8.3v-.8c0-1.7.6-3.2,1.5-4.5.1-.2.3-.4.5-.6s.2-.3.4-.4c0,0,.2-.2.3-.3,1.6-1.6,3.8-2.6,6.2-                    6,0,.2,0,.4,0,.5,0,.4-.3.8-.5,1.2-1.4,2.9-4.5,4.8-7.9,4.8s-8.6-3.8-8.8-8.3v-.8c0-1.6.6-3.2,1.5-4.5.1-.2.3-.4.5-.6s.2-.3.4-.4c0,0,.2-.2.3-.3,1.6-1.6,3.8-2.6,6.2-2.6,4.6,0,8.3,3.5,8.8,8v.8h-.2Z"/>
...
const terminerCreation = () => {
            ...
            const sentierCreationData = JSON.parse(sessionStorage.getItem("sentierCreation"));
            const etapesData = JSON.parse(sessionStorage.getItem("etapes"));

            // Vérifier si les données existent
            if (!sentierCreationData || !etapesData) {
                console.error('Les données du sentier ou des étapes ne sont pas présentes dans le sessionStorage.');
                return;
            }
            if (sessionStorage.getItem('update')) {
                // Envoyer les données au backend
                axios.post('/update-sentier', {
                    ...
                    console.error('Erreur lors de la mise à jour du sentier:', error);
                });
            }else{
                // Envoyer les données au backend
                axios.post('/submit-sentier', {
                    nom: sentierCreationData.nomSentier,
                    description: sentierCreationData.descriptionSentier,
                    theme_id: sentierCreationData.theme,
                    criteres: sentierCreationData.criteres,
