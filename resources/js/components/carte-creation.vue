    ...
    <div id="page">
        <div id="formulaire">
            <div class="form-container">
                ...
                <draggable v-model="etapes" item-key="id">
                    ...
                    <template #item="{ element, index }">
                        <div class="stepContainer">
                            <div class="infoMap">
                                <p v-if="index === etapes.length - 1"><span class="material-symbols-outlined">
                            ...
                            <div class="step" @click="clickEtape(index)">
                                ...
                                <div class="dragHandle"><span class="material-symbols-outlined">
                                        drag_handle
                                    </span></div>
                            </div>
                ...
                <div v-if="etapes.length <= 5" class="add-step" @click="ajouteEtape">+ ajouter une étape</div>
            </div>
        </div>
        <div id="partieInferieur">
            <div id="mapCreationDuSentier">
            </div>
            <div id="recenterDiv">
...
export default {
    ...
    data() {
        return {
            etapes: [
                { nom: 'nom 1', description: 'la description', coordonnees: { long: 6.6832837302582675, lat: 46.575054030719826 }, photo: null, pointInteret: [{ nom: '', photo: null }, { nom: '', photo: null }] },
                { nom: 'nom 2', description: 'la descr', coordonnees: { long: null, lat: null }, photo: null, pointInteret: [{ nom: '', photo: null }, { nom: '', photo: null }] },
                { nom: 'nom 3', description: 'la descr', coordonnees: { long: 6.6832837302582675, lat: 46.585054030719826 }, photo: null, pointInteret: [{ nom: '', photo: null }, { nom: '', photo: null }] },
            ],
            oldIndex: '',
            newIndex: '',
            markers: [],
    ...
    methods: {
        loadMap(tour) {
            ...
            if (tour.length > 1) {
                fetch("https://api.openrouteservice.org/v2/directions/foot-hiking/geojson", {
                    ...
                    body: JSON.stringify({
                        coordinates: tour.map(etape => [etape.coordonnees.long, etape.coordonnees.lat]),
                    }),
                })
        ...
        afficheRoute(tour) {
            ...
            etapesInversees.forEach((etape, index) => {
                ...
                const marker = new maplibregl.Marker({ color: this.couleur }).setLngLat(coordinate).addTo(map);
        ...
        showTours(tour) {
            ...
            if (etapeOk[0]) {
                map.setCenter([etapeOk[etapeOk.length - 1].coordonnees.long, etapeOk[etapeOk.length - 1].coordonnees.lat]);
                map.setZoom(12);
                this.zoomRecenter = 12;
                this.coordonnesRecenter = [etapeOk[etapeOk.length - 1].coordonnees.long, etapeOk[etapeOk.length - 1].coordonnees.lat];
        ...
        ajouteEtape() {
            this.etapes.push({ nom: '', description: '', coordonnees: { long: null, lat: null }, photo: null, pointInteret: [{ nom: '', photo: null }, { nom: '', photo: null }] },
            );
        },
        clickEtape(index) {
