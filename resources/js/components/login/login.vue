...
const submitData = async () => {
  ...
  if (!password.value) {
    ...
    errors.value.password = "Le mot de passe doit avoir au moins 6 caractères.";
    valid = false;
  }
  ...
  if (valid) {
    try {
      ...
      if (response.data.message === 'Authenticated') {
        ...
        errors.value.server = "Erreur de connexion. Veuillez vérifier vos identifiants.";
      }
    } catch (error) {
      errors.value.server = "Erreur de connexion. Veuillez réessayer plus tard.";
      console.error(error);
    }
  }
};
